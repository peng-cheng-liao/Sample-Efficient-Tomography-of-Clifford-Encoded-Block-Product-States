...
def complete_isotropic_to_symplectic(Zcols: np.ndarray) -> np.ndarray:
    """
    Complete an isotropic set of commuting independent Pauli columns
    into a full symplectic tableau.

    Parameters
    ----------
    Zcols : np.ndarray
        Shape (2n, t). Each column is a binary symplectic vector
        [a_1..a_n | b_1..b_n]^T over GF(2), with t <= n.
        The columns must be linearly independent and pairwise commuting.

    Returns
    -------
    F : np.ndarray
        Shape (2n, 2n), a full symplectic tableau
            F = [X_1 ... X_n | Z_1 ... Z_n]
        such that
            F[:, n:n+t] = Zcols .
        In other words, the first t Z-columns are exactly the prescribed
        commuting operators.

    Notes
    -----
    If you set Zcols = [bin(g_1) ... bin(g_t)], then the returned F can be fed
    into synthesize_clifford_from_tableau(F). The synthesized Clifford U will
    satisfy
        U Z_j U^\dagger = g_j    for j = 1,...,t,
    equivalently
        U^\dagger g_j U = Z_j.
    """
    ...
    # isotropic / commuting check: z_i^T J z_j = 0
    ...
    def solve_pairing_constraints(targets: List[np.ndarray], rhs: List[int]) -> np.ndarray:
        """
        Solve for v such that <v, w_i> = rhs_i for each target w_i,
        where <u,v> = u^T J v over GF(2).
        """
        if len(targets) == 0:
            raise ValueError("Internal error: no constraints provided.")

        A = np.vstack([(J @ w) % 2 for w in targets]).astype(np.uint8)
        b = np.array(rhs, dtype=np.uint8)
        return _gf2_solve_one(A, b)

    # Step 1: choose X_1,...,X_t dual to the prescribed Z_1,...,Z_t
    # so that <X_i, Z_j> = delta_{ij} and <X_i, X_k> = 0 for k<i.
    for j in range(t):
    ...
    # Step 2: extend the existing symplectic basis to n pairs
    while len(Z_list) < n:
        W = X_list + Z_list

        # Find any nonzero vector in W^\perp.
        # Since span(W) is already symplectic, any nonzero vector in W^\perp
        # is automatically outside span(W).
        ...
        # Choose x_new such that:
        #   <x_new, w> = 0 for all w in W
        #   <x_new, z_new> = 1
        targets = W + [z_new]
        rhs = [0] * len(W) + [1]
...
def gates_to_unitary(gates: List[Tuple], n: int) -> np.ndarray:
    ...
    Important:
      ...
      right-composition list for the tableau. Therefore we must accumulate
...
def _fix_stabilizer_signs(
    ...
    """
    Given a unitary U that satisfies
        U^dagger g_j U = ± Z_j,   j=1,...,t,
    fix the signs exactly by right-multiplying X_j whenever needed.

    Returns:
      U_fixed, extra_gates

    where extra_gates is a list of ('X', j) gates appended on the right.
    """
    ...
    for j, g in enumerate(g_list):
        ...
        # Since Clifford conjugation of a Hermitian Pauli is ± another Hermitian Pauli,
        # the overlap should be close to ±1 after normalization by 2^n.
        overlap = (Zj * lhs).tr() / (2 ** n)
        overlap = complex(overlap)

        if abs(overlap.imag) > tol:
            raise RuntimeError(
                f"Unexpected non-real phase in sign check for g_{j + 1}; "
                f"overlap={overlap}. Expected approximately ±1."
            )

        if abs(abs(overlap.real) - 1.0) > 1e-5:
            raise RuntimeError(
                f"Sign check failed for g_{j + 1}; overlap={overlap}. "
                f"Expected approximately ±1."
            )

        if overlap.real < 0:
            # Right-multiply by X_j to flip only the sign of Z_j.
...
def _density_to_qobj(rho: Union[np.ndarray, qt.Qobj], n: int) -> qt.Qobj:
    """
    Accept either a NumPy density matrix or a QuTiP Qobj and return
    a normalized n-qubit density operator as a Qobj.
    """
    if isinstance(rho, qt.Qobj):
        return _as_density(rho, n)

    arr = np.asarray(rho, dtype=complex)
    if arr.shape != (2 ** n, 2 ** n):
        raise ValueError(f"rho must have shape {(2 ** n, 2 ** n)} for n={n}.")
...
def _greedy_maximal_independent_commuting_subset(
    ...
    'Maximal' here means no additional candidate can be added while
    preserving both:
      (i) pairwise commutation with the selected set,
      (ii) GF(2)-independence modulo phase.

    The input order is respected, so if `candidates` is score-sorted,
    the returned set is a score-guided maximal subset.
    """
    symp = Symplectic(n)
    span = GF2Basis()
    ...
    for _, P in candidates:
        ...
        # skip if dependent on previously selected operators
        if span.contains(v):
            continue

        # skip if it fails commutation with the selected set
...
def empirical_stabilizer_peeling(
    ...
    Input
    -----
    rho : np.ndarray or qt.Qobj
        n-qubit density matrix.
    n : int
        Number of qubits.
    M1 : int
        Number of Bell-sampling shots. This uses 2*M1 copies of rho.
    lam : float
        Threshold parameter lambda in (0,1). Since `lambda` is a Python
        keyword, this function uses `lam`.
    seed : int or None
        RNG seed passed to bell_sampling.
    return_details : bool
        If True, return a dictionary with intermediate objects.
    ...
    If return_details=False:
        ...
        g_list : list[str]
            A greedy maximal independent commuting subset
            [g_1, ..., g_t] extracted from
                T = { P : \hat s(P) >= 1 - lam }.
        U_stab : np.ndarray
            The synthesized Clifford unitary matrix of shape (2^n, 2^n)
            satisfying
                U_stab^\dagger g_j U_stab = Z_j,   j=1,...,t.

    If return_details=True:
        returns a dict containing g_list, U_stab, gates, tableau, B,
        full ranking, and candidate set.
    """
    if n <= 0:
        raise ValueError("n must be positive.")
    if M1 <= 0:
    ...
    rho_q = _density_to_qobj(rho, n)

    # Step 1: Bell sampling estimator for all P in P_n
    B = bell_sampling(n=n, M=M1, rho=rho_q, seed=seed)
    ranking = rank_all_sP_from_B(B, include_identity=False)

    # Step 2: empirical stabilizer candidate set
    candidates: Ranking = [(score, P) for score, P in ranking if score >= 1.0 - lam]

    # choose a maximal independent commuting subset {g_1,...,g_t}
    g_list = _greedy_maximal_independent_commuting_subset(candidates, n)
    t = len(g_list)

    # Step 3: construct U_stab such that U_stab^\dagger g_j U_stab = Z_j
    if t == 0:
    ...
    # Final sign-fixing pass so that U_stab^\dagger g_j U_stab = +Z_j exactly
...
def _embed_residual_pauli(P_res: str, n: int, t: int) -> str:
    """Embed an m-qubit residual Pauli as I^t ⊗ P_res on n=t+m qubits."""
    m = n - t
    if len(P_res) != m:
...
def _embed_residual_unitary(U_res: np.ndarray, n: int, t: int) -> np.ndarray:
    """Embed an m-qubit unitary as I^(⊗t) ⊗ U_res on n=t+m qubits."""
    m = n - t
    if U_res.shape != (2 ** m, 2 ** m):
        raise ValueError(f"U_res must have shape {(2 ** m, 2 ** m)}.")
    I_left = np.eye(2 ** t, dtype=complex)
    return np.kron(I_left, U_res)
...
def _complete_partial_symplectic_basis(
    ...
    ------
    Xcols : (2m, L)
        Prescribed X-columns for the first L logical qubits.
    Zcols_paired : (2m, a)
        Prescribed Z-columns for the first a logical qubits, where a <= L.

    Required relations
    ------------------
      <X_i, X_j> = 0
      <Z_i, Z_j> = 0
      <X_i, Z_j> = delta_{ij}   for i,j <= a
      <X_j, Z_i> = 0            for j > a, i <= a

    Returns
    -------
    F : (2m, 2m)
        Full symplectic tableau [X_1 ... X_m | Z_1 ... Z_m]
        whose first L X-columns and first a Z-columns equal the prescribed ones.
    """
    ...
    if a > 0:
        ...
        # Singleton X's must commute with prescribed Z's
        if L > a:
            X_single = Xcols[:, a:L]
            cross = (X_single.T @ J @ Zcols_paired) % 2
            if np.any(cross):
    ...
    # For each singleton X_j (j=a,...,L-1), choose a Z_j so that:
    #   <X_k, Z_j> = delta_{k,j} for all prescribed X_k, k=0,...,L-1
    #   <Z_i, Z_j> = 0 for all already-built Z_i
    for j in range(a, L):
        targets = X_list + Z_list
        rhs = [1 if k == j else 0 for k in range(L)] + [0] * len(Z_list)
        zj = solve_pairing_constraints(targets, rhs)
        Z_list.append(zj)

    # Now we have L symplectic pairs. Complete to m pairs.
    while len(X_list) < m:
        W = X_list + Z_list
        A_perp = np.vstack([(J @ w) % 2 for w in W]).astype(np.uint8)
        ns_basis = _gf2_nullspace_basis(A_perp)
        ...
        # Choose z_new dual to x_new and commuting with the existing span.
        targets = W + [x_new]
        rhs = [0] * len(W) + [1]
...
def _fix_recovery_axis_signs(
    ...
    """
    Fix signs so that on the residual m-qubit space:
      U_res^dagger R_i^x U_res = +X_i,  U_res^dagger R_i^z U_res = +Z_i   for i in A1
      U_res^dagger R_j^x U_res = +X_j                                    for j in A2
    where the logical order is [A1 first, then A2].
    """
    Uq = qt.Qobj(U_res, dims=[[2] * m, [2] * m])

    # First the paired qubits A1: fix X-signs by right-multiplying Z_j, then Z-signs by X_j.
    ...
    # Then singleton qubits A2: fix X-sign by right-multiplying Z_j on that logical qubit.
    offset = len(A1_axes)
    for s, axes in enumerate(A2_axes):
        ...
        if overlap_x.real < 0:
            # Use the standard Z_j of that logical qubit to flip X_j's sign.
            U_res = U_res @ _single_qubit_op(m, j, Z2)
            Uq = qt.Qobj(U_res, dims=[[2] * m, [2] * m])
...
def rank_guided_symplectic_recovery(
    ...
    The input state `tilde_rho` is still an n-qubit state, but only the residual
    subsystem on the last m=n-t qubits is used for recovery. The first t qubits are
    untouched throughout.

    Parameters
    ----------
    tilde_rho : np.ndarray or qt.Qobj
        n-qubit state after peeling.
    n : int
        Total number of qubits.
    t : int
        Number of already-peeled / untouched leading qubits.
    M2 : int
        Number of Bell-sampling shots on tilde_rho (uses 2*M2 copies).
    kappa : float
        Threshold in (0, 1/2).
    seed : int or None
        RNG seed for Bell sampling.
    return_details : bool
        If True, return a dict with intermediate ranking/candidates/tableau.

    Returns
    -------
    If return_details=False:
        (recovered_axes, U_rec)

        recovered_axes is a dict with keys:
          "A1_axes_res", "A2_axes_res", "A1_axes_full", "A2_axes_full",
          "A1", "A2", "m"

        U_rec is the full n-qubit unitary matrix satisfying:
          - on A1 (in logical order 1..|A1|):
                U_rec^dagger R_i^x U_rec = X_{t+i}
                U_rec^dagger R_i^z U_rec = Z_{t+i}
          - on A2 (in logical order after A1):
                U_rec^dagger R_j^x U_rec = X_{t+|A1|+j}
        where the X/Z on the RHS act on the residual block, i.e. the first t qubits
        are identity.

    Notes
    -----
    - The recovery is performed only on the residual m=n-t qubits by restricting
      Bell-sampling data to B[t:, :, :].
    - `recover_Q_from_ranking` is not used directly because Algorithm 2 allows
      singleton visible axes (A2), whereas `recover_Q_from_ranking` is the
      all-triples version.
    """
    if n <= 0:
        raise ValueError("n must be positive.")
    ...
    rho_q = _density_to_qobj(tilde_rho, n)

    # Bell sample full n-qubit state, then restrict to the residual last m qubits.
    B_full = bell_sampling(n=n, M=M2, rho=rho_q, seed=seed)
    B_res = B_full[t:, :, :]   # shape (m, M2, 3)
    ...
    # S in the algorithm = span over GF(2) of all accepted axes.
    ...
    for score_x, Px in candidates_res:
        ...
        # Step 5-6: discard if in span(S)
        if span.contains(vx):
            continue

        # Step 8-10: discard if it fails commutation with previously completed qubits
        if not commutes_with_completed_qubits(vx):
            continue

        # Start a new qubit with visible x-axis
        partner = None
        for score_z, Pz in candidates_res:
            ...
            # must anticommute with the current x-axis
            if not symp.anticommutes_int(vz, vx):
                continue

            # must commute with all axes already assigned to other qubits
    ...
    # Build residual tableau:
    #   first |A1| paired qubits, then |A2| singleton qubits.
    a = len(A1_axes_res)
    b = len(A2_axes_res)
    L = a + b
    ...
    if L == 0:
        ...
        Zcols_pairs = np.column_stack(Zcols_pair_list).astype(np.uint8) if a > 0 else np.zeros((2 * m, 0), dtype=np.uint8)
...
def full_recovery_infidelity(
    ...
    (1) empirical_stabilizer_peeling on rho using (M1, lam)
        -> U_stab, t, and tilde_rho = U_stab^dag rho U_stab

    (2) rank_guided_symplectic_recovery on tilde_rho using (M2, kappa)
        -> U_rec

    (3) On
            sigma = U_rec^dag U_stab^dag rho U_stab U_rec,
        perform single-qubit tomography on each qubit using M3 shots per axis.

    (4) Form the product state from the reconstructed single-qubit marginals,
        then rotate it back by U_stab U_rec to obtain rho_recovered.

    Returns
    -------
    If return_details=False:
        infidelity = 1 - F(rho_recovered, rho)^2

    If return_details=True:
        returns a dict containing the intermediate states and unitaries.
    """
    if n <= 0:
        raise ValueError("n must be positive.")
    ...
    tilde_rho = U_stab.dag() * rho_q * U_stab
    ...
    # -------------------------------------------------
    sigma = U_rec.dag() * tilde_rho * U_rec

    rho_prod_prime = [
        single_qubit_tomography(qt.ptrace(sigma, [j]), M3, M3, M3)
        for j in range(n)
    ]
    ...
    # QuTiP fidelity returns the Uhlmann fidelity amplitude,
    # so the usual state fidelity is F^2.
    F_amp = float(qt.fidelity(rho_recovered, rho_q))
    fidelity = F_amp ** 2
    infidelity = 1.0 - fidelity
    trace_distance = 0.5 * float((rho_recovered - rho_q).norm("tr"))
...
def random_clifford_encoded_product_state(
    ...
    where the local single-qubit states rho_i are drawn from four classes:
      - S  : one large Bloch-component squared magnitude in [1-delta, 1], other two 0
      - A1 : all three squared magnitudes in [delta, 1-delta], repeatedly halve the
             largest one until the total norm constraint is satisfied
      - A2 : one squared magnitude in [delta, 1-delta], the other two in [0, delta],
             repeatedly halve the largest one until the total norm constraint is satisfied
      - B  : all three squared magnitudes in [0, delta]

    The class labels are randomly assigned across the n qubit positions.

    Parameters
    ----------
    steps : int
        Number of random Clifford-generator steps used in random_clifford_gate.
    delta : float
        Parameter controlling the allowed Bloch-component magnitudes-squared.
    n : int
        Number of qubits.
    S, A1, A2, B : int
        Numbers of qubits in each class, with S + A1 + A2 + B = n.
    seed : int or None
        RNG seed.
    return_details : bool
        If True, also return local states, Bloch vectors, class assignment, and U_c.

    Returns
    -------
    If return_details=False:
        rho_encoded : qt.Qobj
            The n-qubit density matrix U_c (⊗_i rho_i) U_c^\dagger.

    If return_details=True:
        dict with keys:
          "rho_encoded", "rho_product", "Uc", "Uc_qobj",
          "local_states", "bloch_vectors", "class_assignment"
    """
    if n <= 0:
        raise ValueError("n must be positive.")
    if steps <= 0:
    ...
    def shrink_until_feasible(vals_sq: np.ndarray) -> np.ndarray:
        """
        Repeatedly halve the largest squared magnitude until sum <= 1.
        """
    ...
    def build_local_from_sq(vals_sq: np.ndarray) -> Tuple[qt.Qobj, np.ndarray]:
        """
        Given squared Bloch-component magnitudes [|r_x|^2, |r_y|^2, |r_z|^2],
        assign random signs and build rho = 1/2 (I + r·sigma).
        """
        vals_sq = shrink_until_feasible(vals_sq)
        mags = np.sqrt(np.clip(vals_sq, 0.0, None))
        r = np.array([random_sign() * mags[0],
                      random_sign() * mags[1],
                      random_sign() * mags[2]], dtype=float)
    ...
    def sample_A1() -> Tuple[qt.Qobj, np.ndarray]:
        low, high = float(delta), float(max(delta, 1.0 - delta))
        vals_sq = rng.uniform(low, high, size=3).astype(float)
        return build_local_from_sq(vals_sq)
    ...
    # Randomly distribute the class labels across qubit positions
    ...
    # random_clifford_gate returns (gates, F, U)
...
def random_clifford_encoded_product_pure_state(
    ...
    """
    Generate a random Clifford-encoded PRODUCT PURE state

        |psi_encoded> = U_c (⊗_i |psi_i>)

    with only two classes of local pure states:
      - S  : one |r_i^alpha|^2 is sampled in [1-delta, 1], and the other two
             are small but chosen so that ||r_i||=1 exactly.
      - A1 : all three |r_i^alpha|^2 are at least delta and the Bloch vector is
             pure, i.e. |r_i^x|^2 + |r_i^y|^2 + |r_i^z|^2 = 1.

    Here r_i is the Bloch vector of the pure one-qubit state
        rho_i = (I + r_i^x X + r_i^y Y + r_i^z Z)/2,
    and purity is enforced exactly by requiring ||r_i|| = 1.

    The qubit classes are randomly interleaved across the n sites.
    ...
    steps : int
        Number of random Clifford-generator steps used in random_clifford_gate.
    delta : float
        Parameter controlling the magnitude-squared constraints.
    n : int
        Number of qubits.
    S : int
        Number of qubits in class S.
    A1 : int
        Number of qubits in class A1.
    seed : int or None
        RNG seed.
    return_details : bool
        If True, also return local kets, Bloch vectors, class assignment, and U_c.

    Returns
    -------
    If return_details=False:
        psi_encoded : qt.Qobj
            n-qubit ket vector U_c (⊗_i |psi_i>), returned as a ket.

    If return_details=True:
        dict with keys:
          "psi_encoded", "psi_product", "rho_encoded", "rho_product",
          "Uc", "Uc_qobj", "local_kets", "bloch_vectors", "class_assignment"

    Notes
    -----
    - This function returns a ket, not a density matrix.
    - Requires S + A1 = n.
    - The interval constraints are interpreted on the squared Bloch magnitudes
      exactly as written.
    """
    if n <= 0:
        raise ValueError("n must be positive.")
    ...
    def ket_from_bloch(r: np.ndarray) -> qt.Qobj:
        """
        Build a pure qubit ket from a unit Bloch vector r = (rx, ry, rz).
        """
        rx, ry, rz = map(float, r)
        ...
        # rz = cos(theta), rx + i ry = e^{i phi} sin(theta)
        ...
        ket = qt.Qobj(psi.reshape((2, 1)), dims=[[2], [1]])
        ket = ket.unit()
        return ket

    def sample_S_pure() -> Tuple[qt.Qobj, np.ndarray]:
        """
        One large squared Bloch component in [1-delta, 1], the other two small,
        while preserving purity exactly.
        """
        vals_sq = np.zeros(3, dtype=float)
        big_axis = int(rng.integers(0, 3))
        ...
        # Remaining weight for the two small coordinates
        rem = 1.0 - big_val

        # Split rem randomly between the other two coordinates
        ...
        ket = ket_from_bloch(r)
        return ket, r

    def sample_A1_pure() -> Tuple[qt.Qobj, np.ndarray]:
        """
        Pure-state A1:
          all three |r_alpha|^2 are >= delta,
          sum of squared magnitudes is exactly 1.
        """
        # Sample from the simplex x+y+z=1 with each >= delta
        # Write x = delta + y1, y = delta + y2, z = delta + y3, with y_i >= 0
        # and y1+y2+y3 = 1 - 3 delta.
        rem = 1.0 - 3.0 * delta
        if rem < -1e-12:
        ...
        if rem <= 1e-12:
            ...
            # Dirichlet(1,1,1) for a uniform simplex split
            y = rng.dirichlet(np.ones(3)) * rem
            vals_sq = delta + y
    ...
    # Randomly distribute labels across qubit positions
    labels = (["S"] * S) + (["A1"] * A1)
    rng.shuffle(labels)
    ...
    psi_encoded = Uc_qobj * psi_product
    psi_encoded = psi_encoded.unit()
